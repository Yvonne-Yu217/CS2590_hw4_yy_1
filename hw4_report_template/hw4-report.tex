...
\begin{tabular}{lccc}
  ...
  \midrule
  T5 fine-tuned (best epoch) & 1.93 & 70.17 & 73.81 \\
  \midrule
...
\begin{table}[h!]
  ...
  \begin{tabular}{p{2cm}p{3cm}p{5cm}p{3cm}}
    ...
    \midrule
    Syntax error & malformed SQL query & Parentheses, joins, or clause ordering errors causing SQL execution failure. & 64/466 (13.73\%) \\
    \midrule
    Wrong grounding & wrong city/airline/date values & Query is executable but entities or constants differ from the intent, yielding incorrect records. & 75/466 (16.09\%) \\
    \midrule
    Structural mismatch & partial or over-constrained query & Correct table family but wrong aggregation, filter, or join structure leading to SQL mismatch. & 457/466 (1$-$SQL EM) \\
    \bottomrule
  \end{tabular}
